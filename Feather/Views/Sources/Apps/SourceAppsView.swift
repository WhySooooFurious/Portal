...
struct SourceAppsView: View {
    ...
    private var _navigationTitle: String {
        if object.count == 1 {
            return object.first?.name ?? .localized("Unknown")
        } else {
            return .localized("All Repositories")
        }
    }
    ...
    var body: some View {
        ...
        .toolbarTitleMenu {
            ...
            Button(.localized("Copy"), systemImage: "doc.on.doc") {
                ...
                UIPasteboard.general.string = object.map { $0.sourceURL!.absoluteString }.joined(separator: "\n")
                UIAlertController.showAlertWithOk(
